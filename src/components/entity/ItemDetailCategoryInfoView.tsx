...
export function ItemDetailCategoryInfoView({
  ...
  return (
    ...
    >
        ...
        <div key={chip.label} className="mt-4">
          ...
          <ul className="mt-2 flex flex-wrap gap-1.5">
            {chip.items.map((item) => (
              <li
                key={item}
                className="rounded-md border border-line bg-board px-2.5 py-1 text-sm font-semibold text-ink"
              >
                {item}
              </li>
            ))}
      ...
      {payload.synopsis ? (
        <p className="mt-4 text-base leading-7 text-ink/90">{payload.synopsis}</p>
      ) : null}
      ...
      <div className="mt-4 border-t border-line pt-4">
        ...
        <ul className="mt-2 space-y-2.5">
          {payload.links.map((link) => (
            <li key={link.href} className="text-base leading-snug">
              <a
                href={link.href}
                target="_blank"
                rel="noopener noreferrer"
      ...
      {payload.notice ? (
        <p className="mt-4 rounded-lg bg-board px-3 py-2 text-xs leading-5 text-muted">
          주의: {payload.notice}
        </p>
      ) : null}
    </section>
