...
import { channelHref, channelSectionHref, inferPostChannel, isPostChannel } from "@/lib/posts/channels";
...
export const metadata: Metadata = {
  title: "검색",
  description: `${SITE.name}에서 이슈 칼럼, 브리핑, 지수(INDEX) 항목을 검색합니다.`,
...
export default async function SearchPage({
    ...
    return (
      <div className="space-y-6">
        ...
        <p className="text-sm text-muted">
          찾고 싶은 키워드를 입력하세요. 분석 칼럼, 브리핑, 지수(INDEX) 항목을 함께 찾아 드립니다.
        </p>
  ...
  const [posts, market, briefings] = await Promise.all([
    listPosts(),
    getRankings(),
    Promise.resolve(listSeeded()),
  ]);
  ...
  const total = columns.length + briefingHits.length + entities.length;
