...
export function buildShortsModePrompt(): string {
  return [
    ...
    "",
    "[시제·시의성·노이즈·마침표] 에디션 날짜가 '오늘'. 뉴스 발행일·시의성 라벨 기준 과거형은 과거형만. 에디션보다 이전 일정에 '예정' 금지. 오래된 배경만으로 오늘의 심층 금지. 연도가 다르면 시간순. 'FLOAT' 등 다의어로 이종 산업 기사를 한 인과로 묶지 말고 독립 단락만. 모든 문장 끝 마침표(.) 필수.",
  ].join("\n");
}
