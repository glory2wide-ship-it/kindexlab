 ...
 */
import { stripTrendDisclaimer } from "@/lib/editorial/disclaimer";
import {
...
- ❺ 본문은 KinDex 순위·관심 속도·보드 안 상대 위치 해석만 씁니다. 면책·투자 권유 문구를 본문 어디에든 넣지 마세요.`,
  ]
    .join("\n\n")
    .trim();
}
...
export function buildDataJournalistUserPrompt(params: {
  ...
  return [
    ...
    `- 5번 섹션에 면책·투자 권유 문구를 넣지 마세요. 본문 어디에도 면책 문장을 넣지 마세요.`,
