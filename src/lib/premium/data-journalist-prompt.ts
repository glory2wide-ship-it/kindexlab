...
7. 문장 길이: 한 문장은 공백 제외 45~90자 권장(최소 40자). 20~35자짜리 짧은 단문을 연속으로 쓰지 마세요. 문단당 2~4문장.
8. 문체(높임말 필수): 모든 서술 문장을 '~습니다/~합니다/~됩니다/~있습니다/~없습니다' 등 합쇼체(합니다체)로 끝내세요. '~다/~했다/~이다/~된다' 반말·평서 해라체는 금지입니다. 의문문은 '~까요?'만 허용합니다.
9. 동일 종결 연속 3회 금지: '~습니다'만 반복하지 말고 '~합니다/~됩니다/~었습니다/~고 있습니다' 등으로 리듬을 바꾸세요.
10. 메타 누설 절대 금지: 글자 수, 읽는 시간, SEO, AdSense, '애드센스 고품질 본문 기준 충족', '고품질 본문 기준 충족', LLM 서문을 본문에 넣지 마세요.`;
...
8. 앞으로 지켜볼 흐름 — "가능성이 있습니다/지켜볼 필요가 있습니다" 수준의 신중 전망.

takeaways 3개 = 핵심 요약(독자가 10초 안에 가져갈 포인트, 높임말).
table caption은 '핵심 팩트 요약' 또는 'KinDex 비교'.
title·excerpt·본문·FAQ·takeaways 모두 높임말(합니다체)로 통일하세요.`;
...
export function buildDataJournalistUserPrompt(params: {
  ...
  return [
    ...
    "- 문장은 공백 제외 45~90자로 충분히 길게 쓰세요. 짧은 단문 연속은 금지입니다.",
    "- 모든 문장 끝을 높임말(합니다체: ~습니다/~합니다/~됩니다 등)로 맞추세요. ~다/~했다 해라체 금지.",
    "- takeaways 3개 = 핵심 요약(높임말). table·FAQ는 사실 기반.",
