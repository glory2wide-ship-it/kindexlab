/**
 * One-off: entertainment daily briefing 종합 (main only).
 *
 * Usage:
 *   npx tsx --env-file=.env.local scripts/generate-ent-daily.ts
 *   npx tsx --env-file=.env.local scripts/generate-ent-daily.ts 2026-09-03
 */
...
const SLUG = `${EDITION_DATE}-entertainment-daily`;
...
async function main() {
  ...
  const edition = composeChannelEdition(payload, "entertainment", EDITION_DATE, publishedAt, topicPool);
